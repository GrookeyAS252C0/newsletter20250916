            ...
            for attempt in range(3):
                ...
                response = self.client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": """あなたは天気予報データの専門的な解析者です。学校の登校時間と授業終了時間に特化して、時間帯別の気象データを客観的に抽出してください。

3時間ごとの天気予報データから、以下の情報を抽出してください：

【登校時（8時頃）の情報】
登校時_天気: [8時頃の天気、例：晴れ、曇り、雨]
登校時_最高気温: [8時頃の気温、例：20度]
...
- 客観的なデータのみを抽出してください
- 「快適具合」などの主観的評価は不要です
- 必ず上記すべての項目を含めて回答してください
- データが読み取れない場合は「データなし」と記載してください
- 数値データは可能な限り正確に読み取ってください"""
                        },
                        {
                            "role": "user",
                            "content": [
    ...
    def _parse_screenshot_analysis(self, analysis_text: str) -> Optional[WeatherInfo]:
        ...
        try:
            ...
            # WeatherInfoオブジェクトを作成（快適具合は削除、後でLLMが季節を考慮して生成）
    ...
    def _calculate_analysis_score(self, analysis_text: str) -> float:
        ...
        # 必須フィールドの存在チェック（快適具合は削除）
        required_fields = [
            "登校時_天気", "登校時_最高気温", "登校時_降水確率",
            "授業終了時_天気", "授業終了時_気温", "授業終了時_降水確率"
        ]
